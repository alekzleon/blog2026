...
class AiPostPublisher
{
    public function __construct(
        protected AiPostGenerator $generator,
    ) {
    }

    public function publish(string $topic, ?UploadedFile $image = null): Post
    {
        $generated = $this->generator->generate($topic);
        ...
        $imagePath = $this->resolveImagePath($topic, $generated, $image);
    ...
    protected function resolveImagePath(string $topic, array $generated, ?UploadedFile $image = null): string
    {
        if ($image) {
            return PostImage::storeUpload($image, $topic);
        }
        ...
        try {
            if ($imagePrompt) {
                return $this->generator->generateImage($imagePrompt, $topic);
            }
        } catch (Throwable) {
            //
        }

        return $this->storeDefaultImage($topic);
    }
